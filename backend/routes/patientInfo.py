from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.orm import Session
from backend.database import get_db
from backend.models.database import Patient
from pydantic import BaseModel, Field
from typing import List, Optional

class PatientInfo(BaseModel):
    name: str = Field(..., example="John Doe")
    age: int = Field(..., gt=0, example=70)
    gender: str = Field(..., example="Female")
    education_level: str = Field(..., example="College")
    diabetes: bool = Field(False)
    hypertension: bool = Field(False)
    heart_disease: bool = Field(False)
    stroke_history: bool = Field(False)
    depression: bool = Field(False)
    sleep_disorders: bool = Field(False)
    family_history_dementia: bool = Field(False)
    current_medications: Optional[List[str]] = Field(default_factory=list, example=["metformin", "amlodipine"])

router = APIRouter()

@router.post("/patient-info", response_model=PatientInfo)
def create_patient_info(data: PatientInfo, db: Session = Depends(get_db)):
    """
    Collect and store basic patient information in the database.
    """
    patient = Patient(
        full_name=data.name,
        age=data.age,
        gender=data.gender,
        education_level=data.education_level,
        diabetes=data.diabetes,
        hypertension=data.hypertension,
        heart_disease=data.heart_disease,
        family_history_apoe=data.family_history_dementia,
    )
    db.add(patient)
    db.commit()
    db.refresh(patient)
    return data
